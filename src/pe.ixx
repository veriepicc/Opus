...
public:
    ...
    static Layout compute_layout(bool debug_mode, bool exe_mode = false) {
        ...
        if (debug_mode) {
            l.crash_handler_size = CRASH_HANDLER_SIZE_DEBUG;
            l.repl_handler_size = REPL_HANDLER_SIZE_DEBUG;
            l.repl_handler_offset = crash_offset + l.crash_handler_size;
            l.startup_code_size = l.repl_handler_offset + l.repl_handler_size;
            l.data_extra = BP_TABLE_SIZE + REPL_BUFFER_SIZE + REPL_STATE_SIZE;
            l.has_bp_table = true;
        } else {
            l.crash_handler_size = CRASH_HANDLER_SIZE_RELEASE;
            l.repl_handler_size = 0;
            l.repl_handler_offset = 0;
            l.startup_code_size = crash_offset + l.crash_handler_size;
            l.data_extra = 0;
            l.has_bp_table = false;
        }
        return l;
    }
    ...
    std::vector<std::uint8_t> generate(
        std::vector<std::uint8_t> user_code,
        std::size_t main_offset,
        bool alloc_console = true,
        const std::vector<std::pair<std::size_t, std::size_t>>& iat_fixups = {},
        const std::string& debug_source = "",
        const std::vector<std::pair<std::uint32_t, std::uint32_t>>& line_map = {},
        const std::string& healing_mode = "off",
        bool exe_mode = false,
        ...
        bool has_debug = !debug_source.empty();
        bool has_linemap = has_debug && !line_map.empty();
        bool debug_mode = healing_mode != "off";
        
        // compute layout based on build config
        auto layout = compute_layout(debug_mode, exe_mode);
        ...
        for (const auto& [patch_site, iat_offset] : iat_fixups) {
            // iat lives in .data now, not .rdata
            std::size_t iat_rva = DATA_RVA + iat_offset;
            // Code position RVA (patch_site is offset within user_code)
            std::size_t code_rva = TEXT_RVA + user_code_offset + patch_site + 4;
            // RIP-relative displacement
            std::int32_t disp = static_cast<std::int32_t>(iat_rva) - static_cast<std::int32_t>(code_rva);
            
            if (patch_site + 3 < user_code.size()) {
                user_code[patch_site + 0] = static_cast<std::uint8_t>(disp & 0xFF);
                user_code[patch_site + 1] = static_cast<std::uint8_t>((disp >> 8) & 0xFF);
                user_code[patch_site + 2] = static_cast<std::uint8_t>((disp >> 16) & 0xFF);
                user_code[patch_site + 3] = static_cast<std::uint8_t>((disp >> 24) & 0xFF);
            }
        }
        
        // crash handler needs these
        std::size_t SRC_RVA_FOR_HANDLER = has_debug ? SRC_RVA : 0;
        
        auto imports = build_imports(RDATA_RVA, DATA_RVA);
        auto text = build_code(user_code, main_offset, alloc_console, TEXT_RVA, RDATA_RVA, imports, SRCMAP_RVA, SRC_RVA_FOR_HANDLER, DATA_RVA, healing_mode, layout, exe_mode);
        ...
        std::size_t src_file_sz = has_debug ? align_up(debug_source.size() + 1, kFileAlign) : 0;
        // Line map: 8 bytes per entry (4 bytes offset + 4 bytes line)
        std::size_t srcmap_file_sz = has_linemap ? align_up(line_map.size() * 8 + 4, kFileAlign) : 0;
        ...
        if (!exe_mode) fh.Characteristics |= IMAGE_FILE_DLL;
        append(dll, fh);
        ...
        opt.Subsystem = exe_mode ? IMAGE_SUBSYSTEM_WINDOWS_CUI : IMAGE_SUBSYSTEM_WINDOWS_GUI;
        ...
        if (writable_text) text_sh.Characteristics |= IMAGE_SCN_MEM_WRITE;
        append(dll, text_sh);
        ...
        if (has_debug) {
            ...
            src_sh.Misc.VirtualSize = static_cast<DWORD>(debug_source.size() + 1);
        ...
        if (has_linemap) {
            ...
            srcmap_sh.Misc.VirtualSize = static_cast<DWORD>(line_map.size() * 8 + 4);
        ...
        if (has_linemap) {
            std::uint32_t count = static_cast<std::uint32_t>(line_map.size());
            ...
            for (const auto& [offset, line] : line_map) {
                dll.push_back(static_cast<std::uint8_t>(offset & 0xFF));
                dll.push_back(static_cast<std::uint8_t>((offset >> 8) & 0xFF));
                dll.push_back(static_cast<std::uint8_t>((offset >> 16) & 0xFF));
                dll.push_back(static_cast<std::uint8_t>((offset >> 24) & 0xFF));
                dll.push_back(static_cast<std::uint8_t>(line & 0xFF));
                dll.push_back(static_cast<std::uint8_t>((line >> 8) & 0xFF));
                dll.push_back(static_cast<std::uint8_t>((line >> 16) & 0xFF));
                dll.push_back(static_cast<std::uint8_t>((line >> 24) & 0xFF));
            }
...
private:
    ...
    struct ImportResult {
        ...
        std::size_t str_banner_rva;
        std::size_t banner_len;
    };
    ...
    ImportResult build_imports(std::size_t rdata_rva, std::size_t data_rva) {
        ...
        r.iat_rva = data_rva;
        r.iat_size = static_cast<std::uint32_t>(ilt_size);
        r.iat_create_thread   = data_rva + iat_slot(iat_idx::CreateThread);
        r.iat_alloc_console   = data_rva + iat_slot(iat_idx::AllocConsole);
        r.iat_set_title       = data_rva + iat_slot(iat_idx::SetConsoleTitleA);
        r.iat_get_std_handle  = data_rva + iat_slot(iat_idx::GetStdHandle);
        r.iat_write_console   = data_rva + iat_slot(iat_idx::WriteConsoleA);
        r.iat_sleep           = data_rva + iat_slot(iat_idx::Sleep);
        r.iat_add_veh         = data_rva + iat_slot(iat_idx::AddVectoredExceptionHandler);
        ...
        std::size_t str_banner_off = str_title_off + 0x18;
        std::size_t rdata_needed = str_banner_off + 0x40;  // some padding for strings
        
        r.rdata.resize(rdata_needed, 0);
        
        // write hint+name entries
        for (std::size_t i = 0; i < iat_func_count; i++) {
            std::uint16_t hint = 0;
            memcpy(r.rdata.data() + func_offsets[i], &hint, 2);
            memcpy(r.rdata.data() + func_offsets[i] + 2, iat_functions[i], strlen(iat_functions[i]) + 1);
        }
        ...
        memcpy(r.rdata.data() + ILT_OFFSET, thunks.data(), ilt_size);
        
        // IAT goes into separate iat_data buffer (same content as ILT before loader patches it)
        r.iat_data.resize(ilt_size, 0);
        memcpy(r.iat_data.data(), thunks.data(), ilt_size);
        
        // dll name
        memcpy(r.rdata.data() + dll_name_offset, "kernel32.dll", 13);
        ...
        memcpy(r.rdata.data(), &desc, sizeof(desc));
        
        // string rvas
        r.str_title_rva = rdata_rva + str_title_off;
        r.str_banner_rva = rdata_rva + str_banner_off;
        
        const char* banner = "";
        r.banner_len = strlen(banner);
        memset(r.rdata.data() + str_title_off, 0, 32);
    ...
    std::vector<std::uint8_t> build_code(
        ...
        if (exe_mode) {
            ...
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xEC); emit8(c, 0x20);  // sub rsp, 0x20
            
            // call main
            std::size_t call_pos = c.size();
            std::int32_t main_rel = static_cast<std::int32_t>((user_code_offset + main_offset) - (call_pos + 5));
            emit8(c, 0xE8);
            emit32(c, static_cast<std::uint32_t>(main_rel));
            
            // mov ecx, eax (return value -> ExitProcess arg)
            emit8(c, 0x89); emit8(c, 0xC1);
            
            // call [rip+ExitProcess]
            std::size_t exit_rip = text_rva + c.size() + 6;
            std::size_t exit_iat = data_rva + iat_slot(iat_idx::ExitProcess);
            std::int32_t exit_rel = static_cast<std::int32_t>(exit_iat - exit_rip);
            emit8(c, 0xFF); emit8(c, 0x15);
            emit32(c, static_cast<std::uint32_t>(exit_rel));
            
            emit8(c, 0xCC);  // int3 - should never reach
            
            while (c.size() < EXE_ENTRY_SIZE) emit8(c, 0xCC);
        } else {
            // === DllMain ===
            emit8(c, 0x55);  // push rbp
            emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xE5);  // mov rbp, rsp
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xEC); emit8(c, 0x30);  // sub rsp, 0x30
            
            // save hModule (rcx) before we trash it setting up CreateThread args
            emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x4D); emit8(c, 0xF8);  // mov [rbp-8], rcx
            
            emit8(c, 0x83); emit8(c, 0xFA); emit8(c, 0x01);  // cmp edx, 1
            emit8(c, 0x75);
            std::size_t jne_fixup = c.size();
            emit8(c, 0x00);
            
            if (alloc_console) {
                // set up CreateThread to run users main
                emit8(c, 0x31); emit8(c, 0xC9);  // xor ecx, ecx
                emit8(c, 0x31); emit8(c, 0xD2);  // xor edx, edx
                
                std::size_t lea_pos = c.size();
                std::int32_t rel = static_cast<std::int32_t>(DLLMAIN_SIZE - (lea_pos + 7));
                emit8(c, 0x4C); emit8(c, 0x8D); emit8(c, 0x05);  // lea r8, [rip+...]
                emit32(c, static_cast<std::uint32_t>(rel));
                
                // lpParameter = hModule (saved on stack earlier)
                emit8(c, 0x4C); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF8);  // mov r9, [rbp-8]
                emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0x44); emit8(c, 0x24); emit8(c, 0x20);
                emit32(c, 0);
                emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0x44); emit8(c, 0x24); emit8(c, 0x28);
                emit32(c, 0);
                
                emit_call_iat(c, text_rva, imports.iat_create_thread);
            }
            
            c[jne_fixup] = static_cast<std::uint8_t>(c.size() - jne_fixup - 1);
            emit8(c, 0xB8); emit32(c, 1);  // mov eax, 1
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xC4); emit8(c, 0x30);  // add rsp, 0x30
            emit8(c, 0x5D);  // pop rbp
            emit8(c, 0xC3);  // ret
            
            while (c.size() < DLLMAIN_SIZE) emit8(c, 0xCC);
            ...
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xC4); emit8(c, 0x20);  // add rsp, 0x20
            emit8(c, 0x5D);  // pop rbp
            emit8(c, 0xC3);  // ret
            
            while (c.size() < DLL_PRINT_OFFSET) emit8(c, 0xCC);
        }
        
        // runtime routines - same code, just at different offsets depending on mode
        while (c.size() < print_off) emit8(c, 0xCC);
        
        // === print_impl ===
        std::size_t print_start = c.size();
        build_dll_print_impl(c, text_rva, rdata_rva, imports);
        std::size_t print_bytes = c.size() - print_start;
        
        while (c.size() < set_title_off) emit8(c, 0xCC);
        
        // === set_title_impl ===
        std::size_t title_start = c.size();
        build_dll_set_title_impl(c, text_rva, rdata_rva, imports);
        std::size_t title_bytes = c.size() - title_start;
        
        while (c.size() < alloc_console_off) emit8(c, 0xCC);
        
        // === alloc_console_impl ===
        std::size_t alloc_start = c.size();
        build_dll_alloc_console_impl(c, text_rva, rdata_rva, imports);
        std::size_t alloc_bytes = c.size() - alloc_start;
        
        while (c.size() < print_hex_off) emit8(c, 0xCC);
        
        // === print_hex_impl ===
        std::size_t hex_start = c.size();
        build_dll_print_hex_impl(c, text_rva, rdata_rva, imports);
        std::size_t hex_bytes = c.size() - hex_start;
        ...
        while (c.size() < crash_handler_off) emit8(c, 0xCC);
        
        // === crash handler (VEH) ===
        std::size_t crash_start = c.size();
        if (healing_mode != "off") {
            build_crash_handler(c, text_rva, rdata_rva, imports, user_code_offset, print_off, print_hex_off, srcmap_rva, src_rva, data_rva, healing_mode);
        } else {
            // release mode: catch crash, print info, freeze
            emit8(c, 0x53);                                                     // push rbx
            emit8(c, 0x56);                                                     // push rsi
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xEC); emit8(c, 0x28);    // sub rsp, 0x28

            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x01);                    // mov rax, [rcx] (EXCEPTION_RECORD*)
            emit8(c, 0x8B); emit8(c, 0x18);                                     // mov ebx, [rax] (ExceptionCode)
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x41); emit8(c, 0x08);    // mov rax, [rcx+8] (CONTEXT*)
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0xB0);                    // mov rsi, [rax+0xF8] (RIP)
            emit32(c, 0xF8);

            emit_print_inline(c, text_rva, print_off, "CRASH 0x\0", 9);

            emit8(c, 0x89); emit8(c, 0xD9);                                     // mov ecx, ebx
            std::size_t hex_call1 = c.size();
            std::int32_t hex_rel1 = static_cast<std::int32_t>((text_rva + print_hex_off) - (text_rva + hex_call1 + 5));
            emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(hex_rel1));

            emit_print_inline(c, text_rva, print_off, " @0x\0", 5);

            emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xF1);                    // mov rcx, rsi
            std::size_t hex_call2 = c.size();
            std::int32_t hex_rel2 = static_cast<std::int32_t>((text_rva + print_hex_off) - (text_rva + hex_call2 + 5));
            emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(hex_rel2));

            emit_print_inline(c, text_rva, print_off, "\n\0", 2);

            // freeze so the user can see the output
            emit8(c, 0xEB); emit8(c, 0xFE);                                     // jmp $
        }
        std::size_t crash_bytes = c.size() - crash_start;
        
        while (c.size() < user_code_offset) emit8(c, 0xCC);
        ...
        // === user code ===
        c.insert(c.end(), user_code.begin(), user_code.end());
        return c;
    }
    
    // Embedded dll_print - rcx = null-terminated string
    void build_dll_print_impl(std::vector<std::uint8_t>& c, std::size_t text_rva, 
                               std::size_t rdata_rva, const ImportResult& imports) {
        // push rbp; mov rbp, rsp; sub rsp, 0x30
        emit8(c, 0x55);
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xE5);
        emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xEC); emit8(c, 0x30);
        
        // Save string ptr in [rbp-8]
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x4D); emit8(c, 0xF8);  // mov [rbp-8], rcx
        
        // GetStdHandle(-11) -> stdout handle
        emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0xC1);  // mov rcx, -11
        emit32(c, 0xFFFFFFF5);
        emit_call_iat(c, text_rva, imports.iat_get_std_handle);
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x45); emit8(c, 0xF0);  // mov [rbp-0x10], rax (handle)
        
        // Calculate string length (inline strlen)
        // rax = string pointer, r8d = length counter
        emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xF8);  // mov rax, [rbp-8] (string ptr)
        emit8(c, 0x45); emit8(c, 0x31); emit8(c, 0xC0);  // xor r8d, r8d (length = 0)
        
        // .loop:
        std::size_t loop_start = c.size();
        emit8(c, 0x44); emit8(c, 0x8A); emit8(c, 0x08);  // mov r9b, [rax] (load byte into r9b) - 3 bytes
        emit8(c, 0x45); emit8(c, 0x84); emit8(c, 0xC9);  // test r9b, r9b - 3 bytes
        // je .done (jump past: 3+3+2 = 8 bytes)
        emit8(c, 0x74); emit8(c, 0x08);  // je +8
        emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0xC0);  // inc rax - 3 bytes
        emit8(c, 0x41); emit8(c, 0xFF); emit8(c, 0xC0);  // inc r8d - 3 bytes
        // jmp .loop (back to loop_start)
        emit8(c, 0xEB);  // jmp short
        std::int8_t jmp_back = static_cast<std::int8_t>(loop_start - (c.size() + 1));
        emit8(c, static_cast<std::uint8_t>(jmp_back));
        // .done: r8 now contains the length
        
        // WriteConsoleA(handle, str, len, NULL, NULL)
        emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10] (handle)
        emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x55); emit8(c, 0xF8);  // mov rdx, [rbp-8] (string)
        // r8 already has the length!
        emit8(c, 0x45); emit8(c, 0x31); emit8(c, 0xC9);  // xor r9d, r9d
        emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0x44); emit8(c, 0x24); emit8(c, 0x20);
        emit32(c, 0);
        emit_call_iat(c, text_rva, imports.iat_write_console);
        // Newline is now embedded in the string itself
        
        // add rsp, 0x30; pop rbp; ret
        emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xC4); emit8(c, 0x30);
        emit8(c, 0x5D);
        emit8(c, 0xC3);
    }
    
    // dll_set_title_impl - RCX = string pointer
    void build_dll_set_title_impl(std::vector<std::uint8_t>& c, std::size_t text_rva, 
                                   std::size_t rdata_rva, const ImportResult& imports) {
        emit8(c, 0x55);  // push rbp
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xE5);  // mov rbp, rsp
        emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xEC); emit8(c, 0x20);  // sub rsp, 0x20
        ...
        emit_call_iat(c, text_rva, imports.iat_set_title);
    ...
    void build_dll_alloc_console_impl(std::vector<std::uint8_t>& c, std::size_t text_rva, 
                                       std::size_t rdata_rva, const ImportResult& imports) {
        emit8(c, 0x55);  // push rbp
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xE5);  // mov rbp, rsp
        emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xEC); emit8(c, 0x20);  // sub rsp, 0x20
        ...
        // Call AllocConsole()
        emit_call_iat(c, text_rva, imports.iat_alloc_console);
    ...
    void emit_call_iat(std::vector<std::uint8_t>& c, std::size_t text_rva, std::size_t iat_entry_rva) {
        std::size_t rip_after = text_rva + c.size() + 6;
        std::int32_t rel32 = static_cast<std::int32_t>(iat_entry_rva - rip_after);
        emit8(c, 0xFF); emit8(c, 0x15);
        emit32(c, static_cast<std::uint32_t>(rel32));
    }
    
    void emit_lea_rcx(std::vector<std::uint8_t>& c, std::size_t text_rva, std::size_t target_rva) {
        std::size_t rip_after = text_rva + c.size() + 7;
        std::int32_t rel32 = static_cast<std::int32_t>(target_rva - rip_after);
        emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0x0D);
        emit32(c, static_cast<std::uint32_t>(rel32));
    }
    
    void emit_lea_rdx(std::vector<std::uint8_t>& c, std::size_t text_rva, std::size_t target_rva) {
        std::size_t rip_after = text_rva + c.size() + 7;
        std::int32_t rel32 = static_cast<std::int32_t>(target_rva - rip_after);
        emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0x15);
        emit32(c, static_cast<std::uint32_t>(rel32));
    }
    ...
    //   rsp = rbp - 0x50
    void build_dll_print_hex_impl(std::vector<std::uint8_t>& c, std::size_t text_rva, 
                                   std::size_t rdata_rva, const ImportResult& imports) {
        emit8(c, 0x55);                                     // push rbp
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xE5);    // mov rbp, rsp
        emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xEC); emit8(c, 0x50);  // sub rsp, 0x50
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x4D); emit8(c, 0xF8);  // mov [rbp-8], rcx
        ...
        emit32(c, 0xFFFFFFF5);
        emit_call_iat(c, text_rva, imports.iat_get_std_handle);
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x45); emit8(c, 0xF0);  // mov [rbp-0x10], rax

        // buffer at [rbp-0x22]: 16 hex digits + null (no "0x" prefix, caller handles that)
        // reload value into r8
        emit8(c, 0x4C); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xF8);  // mov r8, [rbp-8]

        for (int i = 0; i < 16; i++) {
            emit8(c, 0x4C); emit8(c, 0x89); emit8(c, 0xC0);                // mov rax, r8
            emit8(c, 0x48); emit8(c, 0xC1); emit8(c, 0xE8); emit8(c, 0x3C); // shr rax, 60
            emit8(c, 0x3C); emit8(c, 0x0A);                                  // cmp al, 10
            emit8(c, 0x72); emit8(c, 0x04);                                  // jb .digit
            emit8(c, 0x04); emit8(c, 0x37);                                  // add al, 0x37
            emit8(c, 0xEB); emit8(c, 0x02);                                  // jmp .store
            emit8(c, 0x04); emit8(c, 0x30);                                  // add al, 0x30
            // digit i goes at [rbp - 0x22 + i]
            int8_t buf_off = static_cast<int8_t>(-0x22 + i);
            emit8(c, 0x88); emit8(c, 0x45); emit8(c, static_cast<uint8_t>(buf_off));
            emit8(c, 0x49); emit8(c, 0xC1); emit8(c, 0xE0); emit8(c, 0x04); // shl r8, 4
        }

        // null terminate: [rbp - 0x22 + 16] = [rbp - 0x12]
        emit8(c, 0xC6); emit8(c, 0x45); emit8(c, 0xEE); emit8(c, 0x00);  // mov byte [rbp-0x12], 0

        // WriteConsoleA(handle, buf, 16, NULL, NULL)
        emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10]
        emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0x55); emit8(c, 0xDE);  // lea rdx, [rbp-0x22]
        emit8(c, 0x41); emit8(c, 0xB8); emit32(c, 16);                    // mov r8d, 16
        emit8(c, 0x45); emit8(c, 0x31); emit8(c, 0xC9);                    // xor r9d, r9d
        emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0x44); emit8(c, 0x24); emit8(c, 0x20);
        emit32(c, 0);                                                       // mov qword [rsp+0x20], 0
        emit_call_iat(c, text_rva, imports.iat_write_console);

        emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xC4); emit8(c, 0x50);  // add rsp, 0x50
        emit8(c, 0x5D);                                                     // pop rbp
        emit8(c, 0xC3);                                                     // ret
    }
    
    // jmp over inline string data, lea rcx to it, call print_impl
    void emit_print_inline(std::vector<std::uint8_t>& c, std::size_t text_rva,
                            std::size_t print_offset, const char* str, std::size_t len) {
        // jmp over the string data
        emit8(c, 0xEB);
        emit8(c, static_cast<std::uint8_t>(len));
        std::size_t str_start = c.size();
        for (std::size_t i = 0; i < len; i++) emit8(c, static_cast<std::uint8_t>(str[i]));
        // lea rcx, [rip + back_to_str]
        std::size_t lea_pos = c.size();
        std::int32_t str_rel = static_cast<std::int32_t>(str_start - (lea_pos + 7));
        emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0x0D); emit32(c, static_cast<std::uint32_t>(str_rel));
        // call print_impl
        std::size_t call_pos = c.size();
        std::int32_t print_rel = static_cast<std::int32_t>((text_rva + print_offset) - (text_rva + call_pos + 5));
        emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(print_rel));
    }
    
    // emit inline string bytes, return position of first byte (caller does the jmp + lea)
    void emit_inline_str(std::vector<std::uint8_t>& c, const std::uint8_t* data, std::size_t len) {
        for (std::size_t i = 0; i < len; i++) emit8(c, data[i]);
    }
    

    // decimal print: value in eax -> stack buffer -> print_impl
    // trashes rax, rcx, rdx, rsi
    void emit_decimal_print(std::vector<std::uint8_t>& c, std::size_t text_rva, std::size_t print_offset) {
        // null terminate at [rbp-0xA1]
        emit8(c, 0xC6); emit8(c, 0x85); emit32(c, 0xFFFFFF5F); emit8(c, 0x00);  // mov byte [rbp-0xA1], 0
        // lea rsi, [rbp-0xA1]
        emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0xB5); emit32(c, 0xFFFFFF5F);
        // mov ecx, 10
        emit8(c, 0xB9); emit32(c, 10);
        // .digit_loop:
        std::size_t digit_loop = c.size();
        emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0xCE);  // dec rsi
        emit8(c, 0x31); emit8(c, 0xD2);                    // xor edx, edx
        emit8(c, 0xF7); emit8(c, 0xF1);                    // div ecx
        emit8(c, 0x80); emit8(c, 0xC2); emit8(c, 0x30);    // add dl, '0'
        emit8(c, 0x88); emit8(c, 0x16);                     // mov [rsi], dl
        emit8(c, 0x85); emit8(c, 0xC0);                     // test eax, eax
        emit8(c, 0x75);                                      // jnz digit_loop
        emit8(c, static_cast<std::uint8_t>(digit_loop - (c.size() + 1)));
        // mov rcx, rsi; call print_impl
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xF1);
        std::size_t cp = c.size();
        std::int32_t pr = static_cast<std::int32_t>((text_rva + print_offset) - (text_rva + cp + 5));
        emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(pr));
    }

    // patch a rel32 at fixup_pos to jump to current c.size()
    static void patch32(std::vector<std::uint8_t>& c, std::size_t fixup_pos) {
        std::int32_t rel = static_cast<std::int32_t>(c.size() - (fixup_pos + 4));
        c[fixup_pos + 0] = static_cast<std::uint8_t>(rel & 0xFF);
        c[fixup_pos + 1] = static_cast<std::uint8_t>((rel >> 8) & 0xFF);
        c[fixup_pos + 2] = static_cast<std::uint8_t>((rel >> 16) & 0xFF);
        c[fixup_pos + 3] = static_cast<std::uint8_t>((rel >> 24) & 0xFF);
    }
    ...
    //   [rbp-0x10] = crash RIP
    //   [rbp-0x18] = CONTEXT*
    void build_crash_handler(std::vector<std::uint8_t>& c, std::size_t text_rva, 
                              std::size_t rdata_rva, const ImportResult& imports,
                              ...
                              const std::string& healing_mode = "off") {
        std::size_t handler_start = c.size();

        // prologue - save all callee-saved regs we use (including r15 for EXCEPTION_RECORD*)
        emit8(c, 0x55);                                     // push rbp
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xE5);    // mov rbp, rsp
        emit8(c, 0x48); emit8(c, 0x81); emit8(c, 0xEC); emit32(c, 0xC0);  // sub rsp, 0xC0
        emit8(c, 0x53);                                     // push rbx
        emit8(c, 0x56);                                     // push rsi
        ...
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x4D); emit8(c, 0xF8);  // mov [rbp-8], rcx
        emit8(c, 0x4C); emit8(c, 0x8B); emit8(c, 0x39);                    // mov r15, [rcx] (EXCEPTION_RECORD*)
        emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x49); emit8(c, 0x08);  // mov rcx, [rcx+8] (CONTEXT*)
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x4D); emit8(c, 0xE8);  // mov [rbp-0x18], rcx (save CONTEXT*)
        emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x81); emit32(c, 0xF8); // mov rax, [rcx+0xF8] (Rip)
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x45); emit8(c, 0xF0);  // mov [rbp-0x10], rax (save crash RIP)

        // compute module base via lea rip trick
        emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0x05); emit32(c, 0);    // lea rax, [rip+0]
        std::size_t rva_after_lea = text_rva + CRASH_HANDLER_OFFSET + (c.size() - handler_start);
        emit8(c, 0x48); emit8(c, 0x2D); emit32(c, static_cast<std::uint32_t>(rva_after_lea));  // sub rax, imm32
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xC3);    // mov rbx, rax (rbx = base)

        // re-entrancy guard: if crash RIP is inside our handler, bail
        // handler lives at [base + TEXT_RVA + CRASH_HANDLER_OFFSET] to [+ CRASH_HANDLER_SIZE]
        emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xF0);  // mov rax, [rbp-0x10] (crash RIP)
        emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xD9);                    // mov rcx, rbx (base)
        emit8(c, 0x48); emit8(c, 0x81); emit8(c, 0xC1);                    // add rcx, handler_start_rva
        emit32(c, static_cast<std::uint32_t>(text_rva + CRASH_HANDLER_OFFSET));
        emit8(c, 0x48); emit8(c, 0x39); emit8(c, 0xC8);                    // cmp rax, rcx
        emit8(c, 0x72);                                                      // jb .not_reentrant (below handler start)
        std::size_t jb_not_reentrant = c.size(); emit8(c, 0x00);
        emit8(c, 0x48); emit8(c, 0x81); emit8(c, 0xC1); emit32(c, CRASH_HANDLER_SIZE);  // add rcx, CRASH_HANDLER_SIZE
        emit8(c, 0x48); emit8(c, 0x39); emit8(c, 0xC8);                    // cmp rax, rcx
        emit8(c, 0x73);                                                      // jae .not_reentrant (above handler end)
        std::size_t jae_not_reentrant = c.size(); emit8(c, 0x00);
        // crash is inside our handler - return EXCEPTION_CONTINUE_SEARCH (0)
        emit8(c, 0x41); emit8(c, 0x5F);                     // pop r15
        emit8(c, 0x41); emit8(c, 0x5E);                     // pop r14
        emit8(c, 0x41); emit8(c, 0x5D);                     // pop r13
        emit8(c, 0x41); emit8(c, 0x5C);                     // pop r12
        emit8(c, 0x5F);                                      // pop rdi
        emit8(c, 0x5E);                                      // pop rsi
        emit8(c, 0x5B);                                      // pop rbx
        emit8(c, 0x48); emit8(c, 0x81); emit8(c, 0xC4); emit32(c, 0xC0);  // add rsp, 0xC0
        emit8(c, 0x5D);                                      // pop rbp
        emit8(c, 0x31); emit8(c, 0xC0);                     // xor eax, eax (return 0)
        emit8(c, 0xC3);                                      // ret
        // .not_reentrant:
        std::size_t not_reentrant = c.size();
        c[jb_not_reentrant] = static_cast<std::uint8_t>(not_reentrant - (jb_not_reentrant + 1));
        c[jae_not_reentrant] = static_cast<std::uint8_t>(not_reentrant - (jae_not_reentrant + 1));

        // print header
        const char hdr[] = "\n=== OPUS CRASH DETECTED ===\n\0";
        emit_print_inline(c, text_rva, print_offset, hdr, sizeof(hdr));

        // ---- exception type detection ----
        // load ExceptionCode from EXCEPTION_RECORD (r15)
        // eax = [r15+0] = ExceptionCode
        emit8(c, 0x41); emit8(c, 0x8B); emit8(c, 0x07);    // mov eax, [r15]

        // compare against known exception codes and branch
        // ACCESS_VIOLATION = 0xC0000005
        emit8(c, 0x3D); emit32(c, 0xC0000005);              // cmp eax, 0xC0000005
        emit8(c, 0x0F); emit8(c, 0x84);                     // je -> handle_av
        std::size_t je_av = c.size(); emit32(c, 0);

        // INTEGER_DIVIDE_BY_ZERO = 0xC0000094
        emit8(c, 0x3D); emit32(c, 0xC0000094);              // cmp eax, 0xC0000094
        emit8(c, 0x0F); emit8(c, 0x84);                     // je -> handle_div_zero
        std::size_t je_div = c.size(); emit32(c, 0);

        // STACK_OVERFLOW = 0xC00000FD
        emit8(c, 0x3D); emit32(c, 0xC00000FD);              // cmp eax, 0xC00000FD
        emit8(c, 0x0F); emit8(c, 0x84);                     // je -> handle_stack_overflow
        std::size_t je_stack = c.size(); emit32(c, 0);

        // ILLEGAL_INSTRUCTION = 0xC000001D
        emit8(c, 0x3D); emit32(c, 0xC000001D);              // cmp eax, 0xC000001D
        emit8(c, 0x0F); emit8(c, 0x84);                     // je -> handle_illegal
        std::size_t je_illegal = c.size(); emit32(c, 0);

        // BREAKPOINT = 0x80000003
        emit8(c, 0x3D); emit32(c, 0x80000003);              // cmp eax, 0x80000003
        emit8(c, 0x0F); emit8(c, 0x84);                     // je -> handle_breakpoint
        std::size_t je_breakpoint = c.size(); emit32(c, 0);

        // SINGLE_STEP = 0x80000004
        emit8(c, 0x3D); emit32(c, 0x80000004);              // cmp eax, 0x80000004
        emit8(c, 0x0F); emit8(c, 0x84);                     // je -> handle_single_step
        std::size_t je_single_step = c.size(); emit32(c, 0);

        // fall through to unknown
        emit8(c, 0xE9);                                      // jmp -> handle_unknown
        std::size_t jmp_unknown = c.size(); emit32(c, 0);

        // ---- .handle_av: ACCESS_VIOLATION ----
        patch32(c, je_av);
        {
            const char av_str[] = "  Exception: ACCESS_VIOLATION (\0";
            emit_print_inline(c, text_rva, print_offset, av_str, sizeof(av_str));
            ...
            emit8(c, 0x41); emit8(c, 0x8B); emit8(c, 0x47); emit8(c, 0x20);  // mov eax, [r15+0x20]
            emit8(c, 0x85); emit8(c, 0xC0);                                    // test eax, eax
            emit8(c, 0x75);                                                     // jnz -> av_write
            std::size_t jnz_av_write = c.size(); emit8(c, 0x00);

            // av read path - tag 1
            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 1);  // mov dword [rbp-0x2C], 1
            const char av_read[] = "read of 0x\0";
            emit_print_inline(c, text_rva, print_offset, av_read, sizeof(av_read));
            emit8(c, 0xEB);                                                     // jmp -> av_addr
            std::size_t jmp_av_addr = c.size(); emit8(c, 0x00);

            // .av_write: tag 2
            std::size_t av_write_label = c.size();
            c[jnz_av_write] = static_cast<std::uint8_t>(av_write_label - (jnz_av_write + 1));
            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 2);  // mov dword [rbp-0x2C], 2
            const char av_write_str[] = "write to 0x\0";
            emit_print_inline(c, text_rva, print_offset, av_write_str, sizeof(av_write_str));

            // .av_addr: print target address from ExceptionInformation[1]
            std::size_t av_addr_label = c.size();
            c[jmp_av_addr] = static_cast<std::uint8_t>(av_addr_label - (jmp_av_addr + 1));

            // load target address into rcx and call print_hex_impl
            emit8(c, 0x49); emit8(c, 0x8B); emit8(c, 0x4F); emit8(c, 0x28);  // mov rcx, [r15+0x28]
            std::size_t hex_call1 = c.size();
            std::int32_t hex_rel1 = static_cast<std::int32_t>((text_rva + print_hex_offset) - (text_rva + hex_call1 + 5));
            emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(hex_rel1));

            const char av_close[] = ")\n\0";
            emit_print_inline(c, text_rva, print_offset, av_close, sizeof(av_close));
        }
        // jmp -> after_exception_type
        emit8(c, 0xE9);
        std::size_t jmp_after_exc_av = c.size(); emit32(c, 0);

        // ---- .handle_div_zero: INTEGER_DIVIDE_BY_ZERO ----
        patch32(c, je_div);
        {
            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 3);  // mov dword [rbp-0x2C], 3 (div-zero tag)
            const char div_str[] = "  Exception: INTEGER_DIVIDE_BY_ZERO\n\0";
            emit_print_inline(c, text_rva, print_offset, div_str, sizeof(div_str));
        }
        emit8(c, 0xE9);
        std::size_t jmp_after_exc_div = c.size(); emit32(c, 0);

        // ---- .handle_stack_overflow: STACK_OVERFLOW ----
        patch32(c, je_stack);
        {
            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 4);  // mov dword [rbp-0x2C], 4 (stack overflow)
            const char so_str[] = "  Exception: STACK_OVERFLOW\n\0";
            emit_print_inline(c, text_rva, print_offset, so_str, sizeof(so_str));
        }
        emit8(c, 0xE9);
        std::size_t jmp_after_exc_so = c.size(); emit32(c, 0);

        // ---- .handle_illegal: ILLEGAL_INSTRUCTION ----
        patch32(c, je_illegal);
        {
            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 5);  // tag 5 (illegal insn)
            const char ill_str[] = "  Exception: ILLEGAL_INSTRUCTION\n\0";
            emit_print_inline(c, text_rva, print_offset, ill_str, sizeof(ill_str));
        }
        emit8(c, 0xE9);
        std::size_t jmp_after_exc_ill = c.size(); emit32(c, 0);

        // ---- .handle_breakpoint: BREAKPOINT ----
        patch32(c, je_breakpoint);
        std::size_t jmp_after_exc_bp_static = 0;
        {
            // check_addr = CONTEXT.RIP (windows VEH points RIP at the INT3, not past it)
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xF0);  // mov rax, [rbp-0x10] (crash RIP = INT3 addr)
            // save check_addr in r14 for bp table scan
            emit8(c, 0x49); emit8(c, 0x89); emit8(c, 0xC6);                    // mov r14, rax
            ...
            emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0xB3);                    // lea rsi, [rbx + bp_table_rva]
            emit32(c, static_cast<std::uint32_t>(data_rva + BP_TABLE_OFFSET_IN_DATA));
            emit8(c, 0xB9); emit32(c, static_cast<std::uint32_t>(BP_TABLE_ENTRIES));  // mov ecx, 16
            emit8(c, 0x31); emit8(c, 0xFF);                                     // xor edi, edi (index counter)

            // .bp_scan_loop:
            std::size_t bp_scan_loop = c.size();
            // check active flag at [rsi + 9]
            emit8(c, 0x80); emit8(c, 0x7E); emit8(c, 0x09); emit8(c, 0x01);  // cmp byte [rsi+9], 1
            emit8(c, 0x75);                                                     // jne .bp_scan_next
            std::size_t jne_bp_scan_next = c.size(); emit8(c, 0x00);
            // active entry - compare address at [rsi+0] with check_addr (r14)
            emit8(c, 0x4C); emit8(c, 0x3B); emit8(c, 0x36);                    // cmp r14, [rsi]
            emit8(c, 0x0F); emit8(c, 0x84);                                     // je .bp_found_dynamic
            std::size_t je_bp_found = c.size(); emit32(c, 0);
            // .bp_scan_next:
            std::size_t bp_scan_next = c.size();
            c[jne_bp_scan_next] = static_cast<std::uint8_t>(bp_scan_next - (jne_bp_scan_next + 1));
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xC6); emit8(c, 0x10);  // add rsi, 16
            emit8(c, 0xFF); emit8(c, 0xC7);                                     // inc edi
            emit8(c, 0xFF); emit8(c, 0xC9);                                     // dec ecx
            emit8(c, 0x75);                                                       // jnz bp_scan_loop
            emit8(c, static_cast<std::uint8_t>(bp_scan_loop - (c.size() + 1)));

            // not found in table -> static breakpoint
            // set bp_type = 1 in repl state
            emit8(c, 0xC6); emit8(c, 0x83);                                     // mov byte [rbx + repl_state_rva], 1
            emit32(c, static_cast<std::uint32_t>(data_rva + REPL_STATE_OFFSET_IN_DATA));
            emit8(c, 0x01);

            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 0);    // mov dword [rbp-0x2C], 0 (unhealable tag)
            const char bp_static_str[] = "  Breakpoint hit (static)\n\0";
            emit_print_inline(c, text_rva, print_offset, bp_static_str, sizeof(bp_static_str));
            emit8(c, 0xE9);                                                     // jmp -> after_exception_type
            jmp_after_exc_bp_static = c.size(); emit32(c, 0);

            // .bp_found_dynamic: rsi points at the matching BP table entry, edi = index
            patch32(c, je_bp_found);

            // save the index as pending_rearm_idx in repl state
            // repl state: byte 0 = bp_type, byte 1 = pending_rearm_idx
            emit8(c, 0x88); emit8(c, 0xBB);                                     // mov [rbx + repl_state_rva + 1], dil
            emit32(c, static_cast<std::uint32_t>(data_rva + REPL_STATE_OFFSET_IN_DATA + 1));

            // set bp_type = 2 (dynamic)
            emit8(c, 0xC6); emit8(c, 0x83);                                     // mov byte [rbx + repl_state_rva], 2
            emit32(c, static_cast<std::uint32_t>(data_rva + REPL_STATE_OFFSET_IN_DATA));
            emit8(c, 0x02);
            ...
            emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x75); emit8(c, 0xD0);  // mov [rbp-0x30], rsi

            // VirtualProtect(check_addr, 1, PAGE_EXECUTE_READWRITE(0x40), &old_protect)
            emit8(c, 0x4C); emit8(c, 0x89); emit8(c, 0xF1);                    // mov rcx, r14 (address)
            emit8(c, 0xBA); emit32(c, 1);                                        // mov edx, 1 (size)
            emit8(c, 0x41); emit8(c, 0xB8); emit32(c, 0x40);                   // mov r8d, 0x40 (PAGE_EXECUTE_READWRITE)
            emit8(c, 0x4C); emit8(c, 0x8D); emit8(c, 0x4D); emit8(c, 0xCC);  // lea r9, [rbp-0x34] (old_protect)
            emit_call_iat(c, text_rva, imports.iat_rva + iat::VirtualProtect);

            // restore rsi
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x75); emit8(c, 0xD0);  // mov rsi, [rbp-0x30]

            // write original byte back: mov al, [rsi+8]; mov [r14], al
            emit8(c, 0x8A); emit8(c, 0x46); emit8(c, 0x08);                    // mov al, [rsi+8] (original_byte)
            emit8(c, 0x41); emit8(c, 0x88); emit8(c, 0x06);                    // mov [r14], al

            // restore original protection
            // VirtualProtect(check_addr, 1, old_protect, &dummy)
            emit8(c, 0x4C); emit8(c, 0x89); emit8(c, 0xF1);                    // mov rcx, r14
            emit8(c, 0xBA); emit32(c, 1);                                        // mov edx, 1
            emit8(c, 0x44); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xCC);  // mov r8d, [rbp-0x34] (old_protect)
            emit8(c, 0x4C); emit8(c, 0x8D); emit8(c, 0x4D); emit8(c, 0xC8);  // lea r9, [rbp-0x38] (dummy)
            emit_call_iat(c, text_rva, imports.iat_rva + iat::VirtualProtect);

            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 0);    // mov dword [rbp-0x2C], 0 (unhealable tag)
            const char bp_dyn_str[] = "  Breakpoint hit (dynamic)\n\0";
            emit_print_inline(c, text_rva, print_offset, bp_dyn_str, sizeof(bp_dyn_str));
        }
        emit8(c, 0xE9);
        std::size_t jmp_after_exc_bp = c.size(); emit32(c, 0);

        // ---- .handle_single_step: STATUS_SINGLE_STEP ----
        // this is transparent - re-arm the dynamic bp and return, no REPL
        patch32(c, je_single_step);
        {
            // load pending_rearm_idx from repl state
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x83);                    // movzx eax, byte [rbx + repl_state_rva + 1]
            emit32(c, static_cast<std::uint32_t>(data_rva + REPL_STATE_OFFSET_IN_DATA + 1));

            // compute bp table entry address: rbx + bp_table_rva + eax*16
            emit8(c, 0xC1); emit8(c, 0xE0); emit8(c, 0x04);                    // shl eax, 4 (index * 16)
            emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0xB3);                    // lea rsi, [rbx + bp_table_rva]
            emit32(c, static_cast<std::uint32_t>(data_rva + BP_TABLE_OFFSET_IN_DATA));
            emit8(c, 0x48); emit8(c, 0x01); emit8(c, 0xC6);                    // add rsi, rax (rsi = entry ptr)

            // load breakpoint address from entry
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x0E);                    // mov rcx, [rsi] (bp address)
            emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x4D); emit8(c, 0xD0);  // mov [rbp-0x30], rcx (save bp addr)
            emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x75); emit8(c, 0xC8);  // mov [rbp-0x38], rsi (save entry ptr)

            // VirtualProtect(bp_addr, 1, PAGE_EXECUTE_READWRITE, &old_protect)
            // rcx already has bp_addr
            emit8(c, 0xBA); emit32(c, 1);                                        // mov edx, 1
            emit8(c, 0x41); emit8(c, 0xB8); emit32(c, 0x40);                   // mov r8d, 0x40
            emit8(c, 0x4C); emit8(c, 0x8D); emit8(c, 0x4D); emit8(c, 0xCC);  // lea r9, [rbp-0x34]
            emit_call_iat(c, text_rva, imports.iat_rva + iat::VirtualProtect);

            // write 0xCC back at the breakpoint address
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xD0);  // mov rax, [rbp-0x30] (bp addr)
            emit8(c, 0xC6); emit8(c, 0x00); emit8(c, 0xCC);                    // mov byte [rax], 0xCC

            // restore protection
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xD0);  // mov rcx, [rbp-0x30]
            emit8(c, 0xBA); emit32(c, 1);                                        // mov edx, 1
            emit8(c, 0x44); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xCC);  // mov r8d, [rbp-0x34]
            emit8(c, 0x4C); emit8(c, 0x8D); emit8(c, 0x4D); emit8(c, 0xC4);  // lea r9, [rbp-0x3C]
            emit_call_iat(c, text_rva, imports.iat_rva + iat::VirtualProtect);

            // clear TF (bit 8) in RFLAGS at CONTEXT offset 0x44
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xE8);  // mov rax, [rbp-0x18] (CONTEXT*)
            emit8(c, 0x8B); emit8(c, 0x48); emit8(c, 0x44);                    // mov ecx, [rax+0x44] (EFlags)
            emit8(c, 0x81); emit8(c, 0xE1); emit32(c, 0xFFFFFEFF);             // and ecx, ~0x100 (clear TF)
            emit8(c, 0x89); emit8(c, 0x48); emit8(c, 0x44);                    // mov [rax+0x44], ecx
            ...
            emit8(c, 0x48); emit8(c, 0x81); emit8(c, 0xC4); emit32(c, 0xC0);  // add rsp, 0xC0
            emit8(c, 0x5D);                                      // pop rbp
            emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0xC0); emit32(c, 0xFFFFFFFF);  // mov rax, -1
            emit8(c, 0xC3);                                      // ret
        }

        // ---- .handle_unknown: UNKNOWN_EXCEPTION (0x...) ----
        patch32(c, jmp_unknown);
        {
            emit8(c, 0xC7); emit8(c, 0x45); emit8(c, 0xD4); emit32(c, 0);  // unhealable
            const char unk_str[] = "  Exception: UNKNOWN_EXCEPTION (0x\0";
            emit_print_inline(c, text_rva, print_offset, unk_str, sizeof(unk_str));

            // print the exception code as hex
            emit8(c, 0x41); emit8(c, 0x8B); emit8(c, 0x07);                    // mov eax, [r15]
            emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0xC1);                    // mov rcx, rax
            std::size_t hex_call2 = c.size();
            std::int32_t hex_rel2 = static_cast<std::int32_t>((text_rva + print_hex_offset) - (text_rva + hex_call2 + 5));
            emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(hex_rel2));

            const char unk_close[] = ")\n\0";
            emit_print_inline(c, text_rva, print_offset, unk_close, sizeof(unk_close));
        }
        // fall through to after_exception_type

        // ---- .after_exception_type: all paths converge here ----
        patch32(c, jmp_after_exc_av);
        patch32(c, jmp_after_exc_div);
        patch32(c, jmp_after_exc_so);
        patch32(c, jmp_after_exc_ill);
        patch32(c, jmp_after_exc_bp);
        patch32(c, jmp_after_exc_bp_static);
        ...
        if (srcmap_rva == 0 || src_rva == 0) {
            ...
            emit_print_inline(c, text_rva, print_offset, no_dbg, sizeof(no_dbg));
            emit8(c, 0xEB); emit8(c, 0xFE);  // jmp $ (freeze)
        } else {
            // ---- crash offset computation ----
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xF0);  // mov rax, [rbp-0x10]
            emit8(c, 0x48); emit8(c, 0x29); emit8(c, 0xD8);                    // sub rax, rbx
            emit8(c, 0x48); emit8(c, 0x2D); emit32(c, 0x1000);                 // sub rax, 0x1000 (TEXT_RVA)
            emit8(c, 0x48); emit8(c, 0x2D); emit32(c, static_cast<std::uint32_t>(user_code_offset));
            emit8(c, 0x49); emit8(c, 0x89); emit8(c, 0xC6);                    // mov r14, rax

            // js -> fallback (crash outside user code)
            emit8(c, 0x0F); emit8(c, 0x88);
            std::size_t js_fallback = c.size(); emit32(c, 0);

            // ---- srcmap linear scan ----
            emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0xB3);                    // lea rsi, [rbx + srcmap_rva]
            emit32(c, static_cast<std::uint32_t>(srcmap_rva));
            emit8(c, 0x8B); emit8(c, 0x0E);                                     // mov ecx, [rsi]
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xC6); emit8(c, 0x04);    // add rsi, 4
            emit8(c, 0x85); emit8(c, 0xC9);                                     // test ecx, ecx
            emit8(c, 0x0F); emit8(c, 0x84);                                     // jz -> no_linemap
            std::size_t jz_nolinemap = c.size(); emit32(c, 0);

            // best_line = 0
            emit8(c, 0x45); emit8(c, 0x31); emit8(c, 0xE4);                    // xor r12d, r12d

            // .scan_loop:
            std::size_t scan_loop = c.size();
            emit8(c, 0x8B); emit8(c, 0x06);                                     // mov eax, [rsi]
            emit8(c, 0x4C); emit8(c, 0x39); emit8(c, 0xF0);                    // cmp rax, r14
            emit8(c, 0x77); emit8(c, 0x04);                                     // ja .skip
            emit8(c, 0x44); emit8(c, 0x8B); emit8(c, 0x66); emit8(c, 0x04);    // mov r12d, [rsi+4]
            // .skip:
            emit8(c, 0x48); emit8(c, 0x83); emit8(c, 0xC6); emit8(c, 0x08);    // add rsi, 8
            emit8(c, 0xFF); emit8(c, 0xC9);                                     // dec ecx
            emit8(c, 0x75);                                                       // jnz scan_loop
            emit8(c, static_cast<std::uint8_t>(scan_loop - (c.size() + 1)));

            // check if we found anything
            emit8(c, 0x45); emit8(c, 0x85); emit8(c, 0xE4);                    // test r12d, r12d
            emit8(c, 0x0F); emit8(c, 0x84);                                     // jz -> no_linemap
            std::size_t jz_nomatch = c.size(); emit32(c, 0);

            // ---- found a line: print location ----
            const char loc[] = "  Location: line \0";
            emit_print_inline(c, text_rva, print_offset, loc, sizeof(loc));
            emit8(c, 0x44); emit8(c, 0x89); emit8(c, 0xE0);                    // mov eax, r12d
            emit_decimal_print(c, text_rva, print_offset);
            const char nl[] = "\n\0";
            emit_print_inline(c, text_rva, print_offset, nl, sizeof(nl));

            // divider before source
            const char div1[] = "  -------------------------------------------\n\0";
            emit_print_inline(c, text_rva, print_offset, div1, sizeof(div1));

            // ---- source context walker ----
            // target_start = max(1, crash_line - 2)
            // mov edi, r12d; sub edi, 2; mov eax, 1; cmp edi, eax; cmovl edi, eax
            emit8(c, 0x44); emit8(c, 0x89); emit8(c, 0xE7);                    // mov edi, r12d
            emit8(c, 0x83); emit8(c, 0xEF); emit8(c, 0x02);                    // sub edi, 2
            emit8(c, 0xB8); emit32(c, 1);                                        // mov eax, 1
            emit8(c, 0x39); emit8(c, 0xC7);                                     // cmp edi, eax
            emit8(c, 0x0F); emit8(c, 0x4C); emit8(c, 0xF8);                    // cmovl edi, eax

            // save target_start in [rbp-0x20]
            emit8(c, 0x89); emit8(c, 0x7D); emit8(c, 0xE0);                    // mov [rbp-0x20], edi

            // target_end = crash_line + 2, save in [rbp-0x24]
            emit8(c, 0x44); emit8(c, 0x89); emit8(c, 0xE0);                    // mov eax, r12d
            emit8(c, 0x83); emit8(c, 0xC0); emit8(c, 0x02);                    // add eax, 2
            emit8(c, 0x89); emit8(c, 0x45); emit8(c, 0xDC);                    // mov [rbp-0x24], eax

            // r13 = base + src_rva (pointer to source text)
            emit8(c, 0x4C); emit8(c, 0x8D); emit8(c, 0xAB);                    // lea r13, [rbx + src_rva]
            emit32(c, static_cast<std::uint32_t>(src_rva));

            // current_line = 1, skip to target_start
            emit8(c, 0xBF); emit32(c, 1);                                        // mov edi, 1

            // .skip_lines: while edi < target_start and *r13 != 0
            std::size_t skip_loop = c.size();
            emit8(c, 0x3B); emit8(c, 0x7D); emit8(c, 0xE0);                    // cmp edi, [rbp-0x20]
            emit8(c, 0x7D);                                                       // jge .done_skip (skip 12 bytes ahead)
            std::size_t jge_done_skip = c.size(); emit8(c, 0x00);               // placeholder
            emit8(c, 0x41); emit8(c, 0x80); emit8(c, 0x7D); emit8(c, 0x00); emit8(c, 0x00); // cmp byte [r13], 0
            emit8(c, 0x74);                                                       // je .done_skip
            std::size_t je_done_skip = c.size(); emit8(c, 0x00);                // placeholder
            // check if byte is '\n'
            emit8(c, 0x41); emit8(c, 0x80); emit8(c, 0x7D); emit8(c, 0x00); emit8(c, 0x0A); // cmp byte [r13], 0x0A
            emit8(c, 0x75); emit8(c, 0x02);                                     // jne .no_newline
            emit8(c, 0xFF); emit8(c, 0xC7);                                     // inc edi
            // .no_newline:
            emit8(c, 0x49); emit8(c, 0xFF); emit8(c, 0xC5);                    // inc r13
            emit8(c, 0xEB);                                                       // jmp skip_loop
            emit8(c, static_cast<std::uint8_t>(skip_loop - (c.size() + 1)));
            // .done_skip:
            std::size_t done_skip = c.size();
            c[jge_done_skip] = static_cast<std::uint8_t>(done_skip - (jge_done_skip + 1));
            c[je_done_skip] = static_cast<std::uint8_t>(done_skip - (je_done_skip + 1));

            // ---- print context lines loop ----
            // edi = current line number, r13 = pointer into source
            // loop while edi <= target_end and *r13 != 0
            std::size_t ctx_loop = c.size();
            emit8(c, 0x3B); emit8(c, 0x7D); emit8(c, 0xDC);                    // cmp edi, [rbp-0x24] (target_end)
            emit8(c, 0x0F); emit8(c, 0x8F);                                     // jg -> done_ctx
            std::size_t jg_done_ctx = c.size(); emit32(c, 0);
            emit8(c, 0x41); emit8(c, 0x80); emit8(c, 0x7D); emit8(c, 0x00); emit8(c, 0x00); // cmp byte [r13], 0
            emit8(c, 0x0F); emit8(c, 0x84);                                     // je -> done_ctx
            std::size_t je_done_ctx = c.size(); emit32(c, 0);

            // print arrow or spaces prefix
            // cmp edi, r12d
            emit8(c, 0x44); emit8(c, 0x39); emit8(c, 0xE7);                    // cmp edi, r12d
            emit8(c, 0x75);                                                       // jne .normal_prefix
            std::size_t jne_normal = c.size(); emit8(c, 0x00);
            // crash line prefix: "> "
            const char arrow[] = "> \0";
            emit_print_inline(c, text_rva, print_offset, arrow, sizeof(arrow));
            emit8(c, 0xEB);                                                       // jmp .after_prefix
            std::size_t jmp_after_prefix = c.size(); emit8(c, 0x00);
            // .normal_prefix:
            std::size_t normal_prefix = c.size();
            c[jne_normal] = static_cast<std::uint8_t>(normal_prefix - (jne_normal + 1));
            const char spaces[] = "  \0";
            emit_print_inline(c, text_rva, print_offset, spaces, sizeof(spaces));
            // .after_prefix:
            std::size_t after_prefix = c.size();
            c[jmp_after_prefix] = static_cast<std::uint8_t>(after_prefix - (jmp_after_prefix + 1));

            // print line number
            emit8(c, 0x89); emit8(c, 0xF8);                                     // mov eax, edi
            // save edi across call
            emit8(c, 0x89); emit8(c, 0x7D); emit8(c, 0xD8);                    // mov [rbp-0x28], edi
            emit_decimal_print(c, text_rva, print_offset);
            emit8(c, 0x8B); emit8(c, 0x7D); emit8(c, 0xD8);                    // mov edi, [rbp-0x28]

            // print " | "
            const char pipe[] = " | \0";
            emit_print_inline(c, text_rva, print_offset, pipe, sizeof(pipe));

            // copy line content from r13 into stack buffer [rbp-0x90], max 80 chars
            // lea rsi, [rbp-0x90]
            emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0xB5); emit32(c, 0xFFFFFF70);
            // xor ecx, ecx (byte counter)
            emit8(c, 0x31); emit8(c, 0xC9);
            // .copy_loop:
            std::size_t copy_loop = c.size();
            emit8(c, 0x41); emit8(c, 0x8A); emit8(c, 0x45); emit8(c, 0x00);    // mov al, [r13]
            emit8(c, 0x3C); emit8(c, 0x00);                                     // cmp al, 0
            emit8(c, 0x74);                                                       // je .end_copy
            std::size_t je_end_copy1 = c.size(); emit8(c, 0x00);
            emit8(c, 0x3C); emit8(c, 0x0A);                                     // cmp al, 0x0A ('\n')
            emit8(c, 0x74);                                                       // je .end_copy
            std::size_t je_end_copy2 = c.size(); emit8(c, 0x00);
            emit8(c, 0x83); emit8(c, 0xF9); emit8(c, 0x4F);                    // cmp ecx, 79
            emit8(c, 0x7D);                                                       // jge .end_copy
            std::size_t jge_end_copy = c.size(); emit8(c, 0x00);
            emit8(c, 0x88); emit8(c, 0x04); emit8(c, 0x0E);                    // mov [rsi+rcx], al
            emit8(c, 0xFF); emit8(c, 0xC1);                                     // inc ecx
            emit8(c, 0x49); emit8(c, 0xFF); emit8(c, 0xC5);                    // inc r13
            emit8(c, 0xEB);                                                       // jmp copy_loop
            emit8(c, static_cast<std::uint8_t>(copy_loop - (c.size() + 1)));
            // .end_copy:
            std::size_t end_copy = c.size();
            c[je_end_copy1] = static_cast<std::uint8_t>(end_copy - (je_end_copy1 + 1));
            c[je_end_copy2] = static_cast<std::uint8_t>(end_copy - (je_end_copy2 + 1));
            c[jge_end_copy] = static_cast<std::uint8_t>(end_copy - (jge_end_copy + 1));

            // append '\n' and null terminator
            emit8(c, 0xC6); emit8(c, 0x04); emit8(c, 0x0E); emit8(c, 0x0A);    // mov byte [rsi+rcx], '\n'
            emit8(c, 0xFF); emit8(c, 0xC1);                                     // inc ecx
            emit8(c, 0xC6); emit8(c, 0x04); emit8(c, 0x0E); emit8(c, 0x00);    // mov byte [rsi+rcx], 0

            // print the line buffer
            // lea rcx, [rbp-0x90]
            emit8(c, 0x48); emit8(c, 0x8D); emit8(c, 0x8D); emit32(c, 0xFFFFFF70);
            std::size_t cp2 = c.size();
            std::int32_t pr2 = static_cast<std::int32_t>((text_rva + print_offset) - (text_rva + cp2 + 5));
            emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(pr2));

            // advance r13 past the newline if we stopped on one
            emit8(c, 0x41); emit8(c, 0x80); emit8(c, 0x7D); emit8(c, 0x00); emit8(c, 0x0A); // cmp byte [r13], '\n'
            emit8(c, 0x75); emit8(c, 0x03);                                     // jne .no_skip_nl
            emit8(c, 0x49); emit8(c, 0xFF); emit8(c, 0xC5);                    // inc r13
            // .no_skip_nl:
            ...
            emit8(c, 0x8B); emit8(c, 0x7D); emit8(c, 0xD8);                    // mov edi, [rbp-0x28]
            emit8(c, 0xFF); emit8(c, 0xC7);                                     // inc edi
            emit8(c, 0x89); emit8(c, 0x7D); emit8(c, 0xD8);                    // mov [rbp-0x28], edi (keep in sync)

            // jmp ctx_loop
            emit8(c, 0xE9);
            std::int32_t ctx_back = static_cast<std::int32_t>(ctx_loop - (c.size() + 4));
            emit32(c, static_cast<std::uint32_t>(ctx_back));

            // .done_ctx:
            patch32(c, jg_done_ctx);
            patch32(c, je_done_ctx);

            // divider after source
            const char div2[] = "  -------------------------------------------\n\0";
            emit_print_inline(c, text_rva, print_offset, div2, sizeof(div2));

            // jmp -> freeze
            emit8(c, 0xE9);
            std::size_t jmp_freeze_main = c.size(); emit32(c, 0);

            // ---- fallback: crash outside user code ----
            patch32(c, js_fallback);
            {
                const char outside[] = "  (crash outside user code)\n\0";
                emit_print_inline(c, text_rva, print_offset, outside, sizeof(outside));
            }
            emit8(c, 0xE9);
            std::size_t jmp_freeze_outside = c.size(); emit32(c, 0);

            // ---- fallback: no line mapping ----
            patch32(c, jz_nolinemap);
            patch32(c, jz_nomatch);
            {
                const char nomap[] = "  (no line mapping available)\n\0";
                emit_print_inline(c, text_rva, print_offset, nomap, sizeof(nomap));
            }

            // ---- register dump ----
            // all paths converge here before freeze
            patch32(c, jmp_freeze_main);
            patch32(c, jmp_freeze_outside);

            // register dump header
            const char reg_hdr[] = "  Registers:\n\0";
            emit_print_inline(c, text_rva, print_offset, reg_hdr, sizeof(reg_hdr));
            ...
            auto emit_reg_value = [&](std::uint32_t ctx_offset) {
                ...
                emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xE8);
                // mov rcx, [rax + ctx_offset]
                emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x88); emit32(c, ctx_offset);
                // call print_hex_impl
                std::size_t call_pos = c.size();
                std::int32_t rel = static_cast<std::int32_t>((text_rva + print_hex_offset) - (text_rva + call_pos + 5));
                emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(rel));
            };
            ...
            auto emit_reg_pair = [&](const char* name1, std::uint32_t off1,
                ...
                emit_print_inline(c, text_rva, print_offset, name1, strlen(name1) + 1);
                emit_reg_value(off1);
                emit_print_inline(c, text_rva, print_offset, name2, strlen(name2) + 1);
                emit_reg_value(off2);
                const char nl[] = "\n\0";
                emit_print_inline(c, text_rva, print_offset, nl, sizeof(nl));
            };
            ...
            emit_reg_pair("  R14 = \0", 0xE8, "  R15 = \0", 0xF0);
            {
                ...
                emit_print_inline(c, text_rva, print_offset, rip_label, sizeof(rip_label));
                emit_reg_value(0xF8);
                const char nl[] = "\n\0";
                emit_print_inline(c, text_rva, print_offset, nl, sizeof(nl));
            }
            ...
            emit8(c, 0xE9);
            std::size_t jmp_over_subs = c.size(); emit32(c, 0);

            // ---- instruction length decoder subroutine ----
            // input: rcx = faulting instruction address
            // output: eax = instruction length in bytes
            std::size_t insn_decoder_offset = c.size() - handler_start;

            emit8(c, 0x31); emit8(c, 0xC0);                    // xor eax, eax
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x11);    // movzx edx, byte [rcx]

            // check REX prefix (0x40-0x4F)
            emit8(c, 0x41); emit8(c, 0x89); emit8(c, 0xD0);    // mov r8d, edx
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xE0); emit8(c, 0xF0); // and r8d, 0xF0
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x40); // cmp r8d, 0x40
            emit8(c, 0x75);                                      // jne .no_rex
            std::size_t jne_norex = c.size(); emit8(c, 0x00);
            emit8(c, 0xFF); emit8(c, 0xC0);                     // inc eax (count REX)
            emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0xC1);    // inc rcx
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x11);    // movzx edx, byte [rcx]
            c[jne_norex] = static_cast<std::uint8_t>(c.size() - (jne_norex + 1));

            // check 0x0F two-byte escape
            emit8(c, 0x80); emit8(c, 0xFA); emit8(c, 0x0F);    // cmp dl, 0x0F
            emit8(c, 0x75);                                      // jne .one_byte
            std::size_t jne_onebyte = c.size(); emit8(c, 0x00);
            emit8(c, 0xFF); emit8(c, 0xC0);                     // inc eax (count 0x0F)
            emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0xC1);    // inc rcx
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x11);    // movzx edx, byte [rcx]
            emit8(c, 0xFF); emit8(c, 0xC0);                     // inc eax (count 2nd opcode byte)
            emit8(c, 0xE9);                                      // jmp .decode_modrm
            std::size_t jmp_modrm = c.size(); emit32(c, 0);

            c[jne_onebyte] = static_cast<std::uint8_t>(c.size() - (jne_onebyte + 1));
            emit8(c, 0xFF); emit8(c, 0xC0);                     // inc eax (count opcode)

            auto emit_opcode_check = [&](uint8_t opcode, std::size_t& target) {
                emit8(c, 0x80); emit8(c, 0xFA); emit8(c, opcode);
                emit8(c, 0x0F); emit8(c, 0x84);
                target = c.size(); emit32(c, 0);
            };
            ...
            for (int i = 0; i < 12; i++) patch32(c, je_modrm[i]);
            emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0xC1);    // inc rcx

            patch32(c, jmp_modrm);
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x11);    // movzx edx, byte [rcx] (ModR/M)
            emit8(c, 0xFF); emit8(c, 0xC0);                     // inc eax (count ModR/M)

            emit8(c, 0x41); emit8(c, 0x89); emit8(c, 0xD0);    // mov r8d, edx
            emit8(c, 0x41); emit8(c, 0xC1); emit8(c, 0xE8); emit8(c, 0x06); // shr r8d, 6
            emit8(c, 0x41); emit8(c, 0x89); emit8(c, 0xD1);    // mov r9d, edx
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xE1); emit8(c, 0x07); // and r9d, 7

            // mod=3: register-register, done
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x03); // cmp r8d, 3
            emit8(c, 0x74);
            std::size_t je_done1 = c.size(); emit8(c, 0x00);

            // SIB check (rm=4 and mod!=3)
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xF9); emit8(c, 0x04); // cmp r9d, 4
            emit8(c, 0x75);
            std::size_t jne_nosib = c.size(); emit8(c, 0x00);
            emit8(c, 0xFF); emit8(c, 0xC0);                     // inc eax (count SIB)
            c[jne_nosib] = static_cast<std::uint8_t>(c.size() - (jne_nosib + 1));

            // displacement
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x00); // cmp r8d, 0
            emit8(c, 0x74);
            std::size_t je_mod00 = c.size(); emit8(c, 0x00);
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x01); // cmp r8d, 1
            emit8(c, 0x74);
            std::size_t je_disp8 = c.size(); emit8(c, 0x00);
            emit8(c, 0x83); emit8(c, 0xC0); emit8(c, 0x04);    // add eax, 4 (mod=2 disp32)
            emit8(c, 0xEB);
            std::size_t jmp_done1 = c.size(); emit8(c, 0x00);

            c[je_mod00] = static_cast<std::uint8_t>(c.size() - (je_mod00 + 1));
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xF9); emit8(c, 0x05); // cmp r9d, 5
            emit8(c, 0x75);
            std::size_t jne_done2 = c.size(); emit8(c, 0x00);
            emit8(c, 0x83); emit8(c, 0xC0); emit8(c, 0x04);    // add eax, 4 (RIP-relative disp32)
            emit8(c, 0xEB);
            std::size_t jmp_done2 = c.size(); emit8(c, 0x00);

            c[je_disp8] = static_cast<std::uint8_t>(c.size() - (je_disp8 + 1));
            emit8(c, 0xFF); emit8(c, 0xC0);                     // inc eax (disp8)

            c[je_done1] = static_cast<std::uint8_t>(c.size() - (je_done1 + 1));
            c[jmp_done1] = static_cast<std::uint8_t>(c.size() - (jmp_done1 + 1));
            c[jne_done2] = static_cast<std::uint8_t>(c.size() - (jne_done2 + 1));
            c[jmp_done2] = static_cast<std::uint8_t>(c.size() - (jmp_done2 + 1));
            emit8(c, 0xC3);                                      // ret

            // ---- destination register decoder subroutine ----
            // input: rcx = faulting instruction address
            // output: eax = CONTEXT offset of destination register
            std::size_t dest_reg_decoder_offset = c.size() - handler_start;

            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x11);    // movzx edx, byte [rcx]
            emit8(c, 0x45); emit8(c, 0x31); emit8(c, 0xC0);    // xor r8d, r8d

            emit8(c, 0x41); emit8(c, 0x89); emit8(c, 0xD1);    // mov r9d, edx
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xE1); emit8(c, 0xF0); // and r9d, 0xF0
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xF9); emit8(c, 0x40); // cmp r9d, 0x40
            emit8(c, 0x75);
            std::size_t jne_norex_dr = c.size(); emit8(c, 0x00);
            emit8(c, 0x0F); emit8(c, 0xBA); emit8(c, 0xE2); emit8(c, 0x02); // bt edx, 2
            emit8(c, 0x41); emit8(c, 0x83); emit8(c, 0xD0); emit8(c, 0x00); // adc r8d, 0
            emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0xC1);    // inc rcx
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x11);    // movzx edx, byte [rcx]
            c[jne_norex_dr] = static_cast<std::uint8_t>(c.size() - (jne_norex_dr + 1));

            emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0xC1);    // inc rcx (skip opcode)
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x11);    // movzx edx, byte [rcx] (ModR/M)

            emit8(c, 0xC1); emit8(c, 0xEA); emit8(c, 0x03);    // shr edx, 3
            emit8(c, 0x83); emit8(c, 0xE2); emit8(c, 0x07);    // and edx, 7

            emit8(c, 0x41); emit8(c, 0xC1); emit8(c, 0xE0); emit8(c, 0x03); // shl r8d, 3
            emit8(c, 0x44); emit8(c, 0x09); emit8(c, 0xC2);    // or edx, r8d

            emit8(c, 0x8D); emit8(c, 0x04); emit8(c, 0xD5); emit32(c, 0x78); // lea eax, [rdx*8 + 0x78]
            emit8(c, 0xC3);                                      // ret

            // patch jmp-over target
            patch32(c, jmp_over_subs);
            ...
            emit8(c, 0x0F); emit8(c, 0xB6); emit8(c, 0x83);                    // movzx eax, byte [rbx + repl_state_rva]
            emit32(c, static_cast<std::uint32_t>(data_rva + REPL_STATE_OFFSET_IN_DATA));
            emit8(c, 0x85); emit8(c, 0xC0);                                      // test eax, eax
            emit8(c, 0x0F); emit8(c, 0x84);                                      // jz -> not_breakpoint (fall through to heal/freeze)
            std::size_t jz_not_bp = c.size(); emit32(c, 0);

            // bp_type == 1 (static): advance RIP by 1, return -1
            emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x01);                    // cmp eax, 1
            emit8(c, 0x75);                                                       // jne -> dynamic_bp_exit
            std::size_t jne_dynamic_bp = c.size(); emit8(c, 0x00);

            // static bp: CONTEXT.RIP points AT the INT3 (windows VEH behavior)
            // advance RIP past the INT3 byte so we dont loop
            emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xE8);  // mov rax, [rbp-0x18] (CONTEXT*)
            emit8(c, 0x48); emit8(c, 0xFF); emit8(c, 0x80); emit32(c, 0xF8); // inc qword [rax+0xF8] (RIP += 1)

            const char bp_cont[] = "  [CONTINUE] resuming after breakpoint\n\0";
            emit_print_inline(c, text_rva, print_offset, bp_cont, sizeof(bp_cont));

            // clear bp_type back to 0
            emit8(c, 0xC6); emit8(c, 0x83);                                     // mov byte [rbx + repl_state_rva], 0
            emit32(c, static_cast<std::uint32_t>(data_rva + REPL_STATE_OFFSET_IN_DATA));
            emit8(c, 0x00);

            // return EXCEPTION_CONTINUE_EXECUTION (-1)
            emit8(c, 0xE9);                                                       // jmp -> bp_return_continue
            std::size_t jmp_bp_return = c.size(); emit32(c, 0);

            // dynamic bp: set trap flag for single-step re-arm, return -1
            c[jne_dynamic_bp] = static_cast<std::uint8_t>(c.size() - (jne_dynamic_bp + 1));
            ...
            emit8(c, 0x8B); emit8(c, 0x48); emit8(c, 0x44);                    // mov ecx, [rax+0x44] (EFlags)
            emit8(c, 0x81); emit8(c, 0xC9); emit32(c, 0x100);                  // or ecx, 0x100 (set TF)
            emit8(c, 0x89); emit8(c, 0x48); emit8(c, 0x44);                    // mov [rax+0x44], ecx

            const char bp_dyn_cont[] = "  [CONTINUE] single-stepping for re-arm\n\0";
            emit_print_inline(c, text_rva, print_offset, bp_dyn_cont, sizeof(bp_dyn_cont));
            ...
            if (healing_mode == "auto") {
                ...
                emit8(c, 0x8B); emit8(c, 0x45); emit8(c, 0xD4);                // mov eax, [rbp-0x2C]
                emit8(c, 0x85); emit8(c, 0xC0);                                  // test eax, eax
                emit8(c, 0x0F); emit8(c, 0x84);                                  // jz -> freeze
                std::size_t jz_freeze = c.size(); emit32(c, 0);

                // ---- healing dispatch ----
                emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x01);                // cmp eax, 1 (av read)
                emit8(c, 0x0F); emit8(c, 0x84);
                std::size_t je_heal_av_read = c.size(); emit32(c, 0);

                emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x02);                // cmp eax, 2 (av write)
                emit8(c, 0x0F); emit8(c, 0x84);
                std::size_t je_heal_av_write = c.size(); emit32(c, 0);

                emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x03);                // cmp eax, 3 (div zero)
                emit8(c, 0x0F); emit8(c, 0x84);
                std::size_t je_heal_div_zero = c.size(); emit32(c, 0);

                emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x04);                // cmp eax, 4 (stack overflow)
                emit8(c, 0x0F); emit8(c, 0x84);
                std::size_t je_heal_stack_overflow = c.size(); emit32(c, 0);

                emit8(c, 0x83); emit8(c, 0xF8); emit8(c, 0x05);                // cmp eax, 5 (illegal insn)
                emit8(c, 0x0F); emit8(c, 0x84);
                std::size_t je_heal_illegal = c.size(); emit32(c, 0);

                // unknown healable tag? shouldnt happen, freeze
                emit8(c, 0xE9);
                std::size_t jmp_freeze_fallback = c.size(); emit32(c, 0);

                // ---- heal AV read: zero dest reg, advance RIP ----
                patch32(c, je_heal_av_read);
                {
                    // call dest_reg_decoder(crash RIP)
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10] (crash RIP)
                    std::size_t call_dr = c.size();
                    std::int32_t dr_rel = static_cast<std::int32_t>((handler_start + dest_reg_decoder_offset) - (call_dr + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(dr_rel));

                    // eax = CONTEXT offset of dest reg, zero it
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xE8);  // mov rcx, [rbp-0x18] (CONTEXT*)
                    emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0x04); emit8(c, 0x01); emit32(c, 0);  // mov qword [rcx+rax], 0

                    // advance RIP
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10]
                    std::size_t call_il = c.size();
                    std::int32_t il_rel = static_cast<std::int32_t>((handler_start + insn_decoder_offset) - (call_il + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(il_rel));

                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xE8);  // mov rcx, [rbp-0x18] (CONTEXT*)
                    emit8(c, 0x48); emit8(c, 0x98);                                    // cdqe
                    emit8(c, 0x48); emit8(c, 0x01); emit8(c, 0x81); emit32(c, 0xF8);  // add [rcx+0xF8], rax

                    // check if target address >= 0x10000 for non-null variant message
                    emit8(c, 0x49); emit8(c, 0x8B); emit8(c, 0x4F); emit8(c, 0x28);  // mov rcx, [r15+0x28] (target addr)
                    emit8(c, 0x48); emit8(c, 0x81); emit8(c, 0xF9); emit32(c, 0x10000);  // cmp rcx, 0x10000
                    emit8(c, 0x0F); emit8(c, 0x83);                                    // jae -> non-null read msg
                    std::size_t jae_nonnull_read = c.size(); emit32(c, 0);

                    const char heal_read[] = "  [HEAL] null read -> zeroed dest register, continuing\n\0";
                    emit_print_inline(c, text_rva, print_offset, heal_read, sizeof(heal_read));
                    emit8(c, 0xE9);
                    std::size_t jmp_read_done = c.size(); emit32(c, 0);

                    // non-null read: print address in the message
                    patch32(c, jae_nonnull_read);
                    const char heal_av_read_pre[] = "  [HEAL] access violation read at 0x\0";
                    emit_print_inline(c, text_rva, print_offset, heal_av_read_pre, sizeof(heal_av_read_pre));
                    emit8(c, 0x49); emit8(c, 0x8B); emit8(c, 0x4F); emit8(c, 0x28);  // mov rcx, [r15+0x28]
                    std::size_t hx_r1 = c.size();
                    std::int32_t hx_r1_rel = static_cast<std::int32_t>((text_rva + print_hex_offset) - (text_rva + hx_r1 + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(hx_r1_rel));
                    const char heal_av_read_post[] = " -> zeroed dest register, continuing\n\0";
                    emit_print_inline(c, text_rva, print_offset, heal_av_read_post, sizeof(heal_av_read_post));

                    patch32(c, jmp_read_done);
                }
                emit8(c, 0xE9);
                std::size_t jmp_continue1 = c.size(); emit32(c, 0);

                // ---- heal AV write: skip the instruction ----
                patch32(c, je_heal_av_write);
                {
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10]
                    std::size_t call_il2 = c.size();
                    std::int32_t il_rel2 = static_cast<std::int32_t>((handler_start + insn_decoder_offset) - (call_il2 + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(il_rel2));

                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xE8);  // mov rcx, [rbp-0x18]
                    emit8(c, 0x48); emit8(c, 0x98);                                    // cdqe
                    emit8(c, 0x48); emit8(c, 0x01); emit8(c, 0x81); emit32(c, 0xF8);  // add [rcx+0xF8], rax
                    ...
                    emit8(c, 0x49); emit8(c, 0x8B); emit8(c, 0x4F); emit8(c, 0x28);  // mov rcx, [r15+0x28]
                    emit8(c, 0x48); emit8(c, 0x81); emit8(c, 0xF9); emit32(c, 0x10000);  // cmp rcx, 0x10000
                    emit8(c, 0x0F); emit8(c, 0x83);                                    // jae -> non-null write msg
                    std::size_t jae_nonnull_write = c.size(); emit32(c, 0);

                    const char heal_write[] = "  [HEAL] null write -> skipped instruction, continuing\n\0";
                    emit_print_inline(c, text_rva, print_offset, heal_write, sizeof(heal_write));
                    emit8(c, 0xE9);
                    std::size_t jmp_write_done = c.size(); emit32(c, 0);

                    // non-null write: print address in the message
                    patch32(c, jae_nonnull_write);
                    const char heal_av_write_pre[] = "  [HEAL] access violation write at 0x\0";
                    emit_print_inline(c, text_rva, print_offset, heal_av_write_pre, sizeof(heal_av_write_pre));
                    emit8(c, 0x49); emit8(c, 0x8B); emit8(c, 0x4F); emit8(c, 0x28);  // mov rcx, [r15+0x28]
                    std::size_t hx_w1 = c.size();
                    std::int32_t hx_w1_rel = static_cast<std::int32_t>((text_rva + print_hex_offset) - (text_rva + hx_w1 + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(hx_w1_rel));
                    const char heal_av_write_post[] = " -> skipped instruction, continuing\n\0";
                    emit_print_inline(c, text_rva, print_offset, heal_av_write_post, sizeof(heal_av_write_post));

                    patch32(c, jmp_write_done);
                }
                emit8(c, 0xE9);
                std::size_t jmp_continue2 = c.size(); emit32(c, 0);

                // ---- heal div-by-zero: zero RAX and RDX, advance RIP ----
                patch32(c, je_heal_div_zero);
                {
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xE8);  // mov rcx, [rbp-0x18] (CONTEXT*)
                    // zero RAX in CONTEXT (offset 0x78)
                    emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0x81); emit32(c, 0x78); emit32(c, 0);
                    // zero RDX in CONTEXT (offset 0x88)
                    emit8(c, 0x48); emit8(c, 0xC7); emit8(c, 0x81); emit32(c, 0x88); emit32(c, 0);

                    // advance RIP
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10]
                    std::size_t call_il3 = c.size();
                    std::int32_t il_rel3 = static_cast<std::int32_t>((handler_start + insn_decoder_offset) - (call_il3 + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(il_rel3));
                    ...
                    emit_print_inline(c, text_rva, print_offset, heal_div, sizeof(heal_div));
                }
                emit8(c, 0xE9);
                std::size_t jmp_continue3 = c.size(); emit32(c, 0);

                // ---- heal stack overflow: reset RSP, skip instruction ----
                patch32(c, je_heal_stack_overflow);
                {
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xE8);  // mov rcx, [rbp-0x18] (CONTEXT*)

                    // reset RSP to RBP from CONTEXT - gives us a sane stack frame
                    // the overflowed function is toast but execution can continue
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x81); emit32(c, 0xA0);  // mov rax, [rcx+0xA0] (CONTEXT.Rbp)
                    emit8(c, 0x48); emit8(c, 0x89); emit8(c, 0x81); emit32(c, 0x98);  // mov [rcx+0x98], rax (CONTEXT.Rsp = Rbp)

                    // advance RIP past the faulting instruction
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10] (crash RIP)
                    std::size_t call_il_so = c.size();
                    std::int32_t il_rel_so = static_cast<std::int32_t>((handler_start + insn_decoder_offset) - (call_il_so + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(il_rel_so));
                    ...
                    emit_print_inline(c, text_rva, print_offset, heal_so, sizeof(heal_so));
                }
                emit8(c, 0xE9);
                std::size_t jmp_continue4 = c.size(); emit32(c, 0);

                // ---- heal illegal instruction: skip it ----
                patch32(c, je_heal_illegal);
                {
                    // just skip past the bad opcode
                    emit8(c, 0x48); emit8(c, 0x8B); emit8(c, 0x4D); emit8(c, 0xF0);  // mov rcx, [rbp-0x10]
                    std::size_t call_il_ill = c.size();
                    std::int32_t il_rel_ill = static_cast<std::int32_t>((handler_start + insn_decoder_offset) - (call_il_ill + 5));
                    emit8(c, 0xE8); emit32(c, static_cast<std::uint32_t>(il_rel_ill));
                    ...
                    emit_print_inline(c, text_rva, print_offset, heal_ill, sizeof(heal_ill));
                }
                // fall through to continue_execution

                // ---- .continue_execution: return EXCEPTION_CONTINUE_EXECUTION ----
                patch32(c, jmp_continue1);
                patch32(c, jmp_continue2);
                patch32(c, jmp_continue3);
                patch32(c, jmp_continue4);
                ...
                patch32(c, jz_freeze);
                patch32(c, jmp_freeze_fallback);
                const char freeze_msg[] = "  [PAUSED] attach debugger...\n\0";
                emit_print_inline(c, text_rva, print_offset, freeze_msg, sizeof(freeze_msg));
                emit8(c, 0xEB); emit8(c, 0xFE);  // jmp $
            } else {
        ...
        std::size_t handler_bytes = c.size() - handler_start;
